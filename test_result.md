...
test_plan:
  current_focus:
    - "See who liked a post/comment (LikersSheet)"
    - "Notification bell on Discover/Log/Profile"
  stuck_tasks: []
  test_all: false
...
agent_communication:
    -agent: "main"
    -message: "THREE FEATURE ADDITIONS (frontend-only; backend liker endpoints already existed). Please WEB-verify (login reese@teebox.demo/password123): (1) LIKERS LIST - On the Feed, find a round card that has at least 1 like (like count > 0). Tap the like COUNT number next to the heart (data-testid='round-card-like-count-<id>') -> a bottom sheet (data-testid='likers-sheet') opens listing users who liked it; tapping a user (data-testid='liker-<userId>') navigates to their profile. NOTE: tapping the heart itself (data-testid='round-card-like-<id>') should still TOGGLE the like, NOT open the sheet. If no card has likes, like one first, then tap the count. (2) LIKERS ON POST DETAIL - Open a post detail (/post/<id>). Tap the 'N likes' text (data-testid='post-like-count') -> likers sheet opens. The heart (data-testid='post-like') still toggles. Also, on a comment that has likes, tap the comment like count (data-testid='comment-like-count-<commentId>') -> likers sheet opens showing who liked that comment; the comment heart (data-testid='comment-like-<commentId>') still toggles. (3) NOTIFICATION BELL - Confirm a bell icon (data-testid='header-notifications', plus 'profile-notifications' on Profile) now appears in the header of Discover (top-right of title), Log (top-right of title), and Profile (in the cover top bar). Tapping it navigates to /notifications. Report any console errors, navigation bugs, or sheets not opening. Base URL: web preview port 3000."
    -agent: "testing"
